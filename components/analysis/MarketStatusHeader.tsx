			...
			<CardContent className="p-6">
				...
				<div className="flex items-center justify-between mb-6">
					<div className="flex items-center space-x-3">
						...
						<div>
							<h3 className="text-lg font-bold text-foreground">실시간 시장 동향</h3>
							<div className="flex items-center space-x-2">
								...
								<Badge variant="secondary" className="text-xs px-2 py-1 rounded-full bg-green-100 dark:bg-green-950/20 text-green-700 dark:text-green-400 border-0">
									LIVE
								</Badge>
							</div>
						</div>
								...
								return (
									<div key={index} className="bg-white/50 dark:bg-gray-800/50 rounded-xl p-4 border border-gray-200/50 dark:border-gray-700/50 backdrop-blur-sm">
										<div className="flex items-start justify-between mb-2">
											<div>
												<h4 className="font-semibold text-foreground text-sm">{market.name}</h4>
												<p className="text-xs text-muted-foreground">{market.symbol}</p>
											</div>
											<div className={`p-1 rounded-lg ${market.change_percent >= 0 ? "bg-green-100 dark:bg-green-950/20" : "bg-red-100 dark:bg-red-950/20"}`}>
												<ChangeIcon className={`h-3 w-3 ${getChangeColor(market.change_percent)}`} />
											</div>
										</div>
										<div className="space-y-1">
											...
											<div className={`text-sm font-medium ${getChangeColor(market.change_percent)}`}>

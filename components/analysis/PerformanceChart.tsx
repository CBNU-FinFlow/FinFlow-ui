			...
			<CardHeader className="pb-4">
				<div className="flex items-center justify-between">
					<div className="flex items-center space-x-2">
						<CardTitle className="text-lg font-bold text-foreground">포트폴리오 성과 시뮬레이션</CardTitle>
						<HelpTooltip
					...
					<div className="flex items-center space-x-2">
						<Badge className="bg-green-100 text-green-700 dark:bg-green-950 dark:text-green-400 border-0">+{quickMetrics.annualReturn}</Badge>
				...
				<CardDescription className="text-muted-foreground">1년간 예상 수익률 추이 및 벤치마크 비교</CardDescription>
			</CardHeader>
			<CardContent className="pt-0">
				{/* 모의 차트 영역 */}
				<div className="h-56 bg-gradient-to-br from-muted/30 to-muted/40 dark:from-gray-800/50 dark:to-gray-900/50 rounded-lg p-4 relative overflow-hidden">
					{/* 차트 배경 그리드 */}
					<div className="absolute inset-0 opacity-20">
						{[...Array(12)].map((_, i) => (
							<div key={i} className="absolute border-l border-border" style={{ left: `${(i + 1) * 8.33}%`, height: "100%" }}></div>
						))}
						{[...Array(6)].map((_, i) => (
							<div key={i} className="absolute border-t border-border w-full" style={{ top: `${(i + 1) * 16.67}%` }}></div>
						))}
					</div>
					...
					<div className="absolute bottom-2 left-4 flex items-center space-x-4 text-xs">
						<div className="flex items-center space-x-1">
							...
							<span className="text-muted-foreground">내 포트폴리오</span>
						</div>
						<div className="flex items-center space-x-1">
							<div className="w-3 h-0.5 bg-muted-foreground/50 border-dashed border-t"></div>
							<span className="text-muted-foreground">S&P 500</span>
						</div>
					</div>

					{/* 현재 값 표시 */}
					<div className="absolute top-4 right-4 bg-card/90 backdrop-blur-sm rounded-lg p-3 shadow-sm border border-border">
						<div className="text-lg font-bold text-foreground">{(Number(investmentAmount) * (1 + parseFloat(quickMetrics.annualReturn.replace("%", "")) / 100)).toLocaleString()}원</div>
				...
				<div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-4">
					<div className="text-center p-3 bg-muted/30 dark:bg-gray-800/50 rounded-lg">
						<div className="text-lg font-bold text-blue-600">{quickMetrics.annualReturn}</div>
						<div className="text-xs text-muted-foreground">연간 수익률</div>
					</div>
					<div className="text-center p-3 bg-muted/30 dark:bg-gray-800/50 rounded-lg">
					...
					<div className="text-center p-3 bg-muted/30 dark:bg-gray-800/50 rounded-lg">
						...
						<div className="text-xs text-muted-foreground">최대 낙폭</div>
					</div>
					<div className="text-center p-3 bg-muted/30 dark:bg-gray-800/50 rounded-lg">
